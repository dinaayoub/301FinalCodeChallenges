...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(person => {
    let filledTemplate = $template.clone();
    filledTemplate.children('h2').text(person.name);
    filledTemplate.children('h3').text(person.height);
    filledTemplate.children('p').text(person.eye_color);
    html.append(filledTemplate);
  })
}
